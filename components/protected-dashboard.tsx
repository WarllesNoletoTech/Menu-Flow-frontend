...
export function ProtectedDashboard({ role }: { role: Role }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001'}/auth/me`, { headers: { Authorization: `Bearer ${current.accessToken}` } });
