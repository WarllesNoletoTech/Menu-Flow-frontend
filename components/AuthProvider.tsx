...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => { const session = getSession(); if (!session) { setUser(null); setLoading(false); return; } setUser(session.user); setAuthError(''); try { const verified = await authenticatedRequest<User>('/auth/me'); saveSession({ ...session, user: verified }); setUser(verified); } catch (error) { if (error instanceof ApiError && error.status === 401) { clearSession(); setUser(null); } else setAuthError(error instanceof Error ? error.message : 'Não foi possível validar a sessão agora.'); } finally { setLoading(false); } }, []);
  useEffect(() => { void refresh(); }, [refresh]);
  return <AuthContext.Provider value={{ user, loading, authError, retry: refresh, login: (session) => { saveSession(session); setAuthError(''); setUser(session.user); }, logout: () => { const session = getSession(); if (session?.accessToken) void detachPushSubscriptionOnLogout(session.accessToken); clearSession(); setUser(null); setAuthError(''); } }}>{children}</AuthContext.Provider>;
}
