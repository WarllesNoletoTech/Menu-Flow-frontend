...
export const orderStatus: Record<string,string> = {
  ...
  READY:'Pronto', DELIVERED_TO_TABLE:'Entregue na mesa', OUT_FOR_DELIVERY:'Saiu para entrega', COMPLETED:'Concluído',
...
const actions: Record<string,Array<[string,string]>> = {
  ...
  READY:[['DELIVERED_TO_TABLE','Entregue na mesa'],['OUT_FOR_DELIVERY','Saiu para entrega'],['COMPLETED','Concluir'],['CANCELLED','Cancelar pedido']],
  DELIVERED_TO_TABLE:[],
...
const groups = {
  ...
  'in-progress':{label:'Em andamento',api:'in_progress',statuses:['ACCEPTED','PREPARING','READY','DELIVERED_TO_TABLE','OUT_FOR_DELIVERY'],empty:'Nenhum pedido em andamento.'},
...
function statusClass(status:string){if(['COMPLETED','READY','DELIVERED_TO_TABLE'].includes(status))return'bg-success/10 text-success';if(['REJECTED','CANCELLED'].includes(status))return'bg-danger/10 text-danger';if(['PENDING','PREPARING'].includes(status))return'bg-accent/15 text-accent';return'bg-background text-ink'}
