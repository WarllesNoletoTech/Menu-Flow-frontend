...
function CheckoutModal({ restaurant, cart, subtotal, addonPrice, setQuantity, close, submit, fulfillment, setFulfillment, customer, submitting }: { restaurant: Restaurant; cart: CartItem[]; subtotal: number; addonPrice: (p: Product, n: string[]) => number; setQuantity: (item: CartItem, quantity: number) => void; close: () => void; submit: (event: FormEvent<HTMLFormElement>) => void; fulfillment: 'PICKUP' | 'DELIVERY'; setFulfillment: (value: 'PICKUP' | 'DELIVERY') => void; customer?: { name: string; phone?: string }; submitting: boolean }) {
  ...
  useEffect(() => {
    if (fulfillment === 'PICKUP' && !restaurant.pickupEnabled && restaurant.deliveryAvailable) setFulfillment('DELIVERY');
    if (fulfillment === 'DELIVERY' && !restaurant.deliveryAvailable && restaurant.pickupEnabled) setFulfillment('PICKUP');
  }, [fulfillment, restaurant.deliveryAvailable, restaurant.pickupEnabled, setFulfillment]);
  ...
  return <ModalFrame label="Finalizar pedido" close={close} sheet><form className="pb-3" onSubmit={submit} onInput={(event) => { const form = event.currentTarget; setAddressComplete(['zipCode','street','number','city','state'].every(field => String(new FormData(form).get(field) ?? '').trim().length > 0)); }}><div className="flex items-center justify-between gap-4"><h2 className="text-xl font-black sm:text-2xl">Finalizar pedido</h2><button type="button" onClick={close} aria-label="Fechar checkout" className="grid h-11 w-11 place-items-center rounded-xl border">✕</button></div>
    ...
    <CheckoutSection number="3" title="Como receber"><div className="grid gap-3 sm:grid-cols-2">{restaurant.pickupEnabled&&<label className={`rounded-xl border p-4 font-bold ${fulfillment==='PICKUP'?'border-ink bg-lime/20':''}`}><input className="mr-2" type="radio" name="fulfillmentChoice" checked={fulfillment==='PICKUP'} onChange={()=>setFulfillment('PICKUP')}/>Retirar no estabelecimento</label>}{restaurant.deliveryAvailable&&<label className={`rounded-xl border p-4 font-bold ${fulfillment==='DELIVERY'?'border-ink bg-lime/20':''}`}><input className="mr-2" type="radio" name="fulfillmentChoice" checked={fulfillment==='DELIVERY'} onChange={()=>setFulfillment('DELIVERY')}/>Entrega</label>}</div>{restaurant.deliveryEnabled&&!restaurant.deliveryAvailable&&<p className="mt-3 text-sm font-semibold text-stone-500">Entrega temporariamente indisponível.</p>}</CheckoutSection>
