...
export function Menu({ slug }: { slug: string }) {
  ...
  return (
    <main className={`min-h-screen bg-stone-100 ${cart.length ? 'pb-28 lg:pb-10' : 'pb-10'}`}>
    ...
    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/80 to-transparent p-5 pt-20 text-white sm:p-8 sm:pr-72 lg:p-10 lg:pr-80">
      <div className="flex items-end gap-4 sm:gap-6">
        ...
        <div className="min-w-0 pb-1">
          <span className={`inline-block rounded-full px-3 py-1 text-xs font-black ${restaurant.open ? 'bg-lime text-ink' : 'bg-red-500 text-white'}`}>{restaurant.open ? 'ABERTO AGORA' : 'FECHADO'}</span>
...
function ProductCard({ product, open, add }: { product: Product; open: boolean; add: () => void }) {
  return <article className="flex min-h-36 min-w-0 gap-3 rounded-2xl border border-stone-200 bg-white p-3 shadow-sm transition hover:shadow-soft sm:gap-4 sm:p-4">
...
function CartPanel({ cart, subtotal, addonPrice, setQuantity, checkout }: { cart: CartItem[]; subtotal: number; addonPrice: (p: Product, n: string[]) => number; setQuantity: (item: CartItem, quantity: number) => void; checkout: () => void }) {
  return <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-soft"><h2 className="text-xl font-black">Sua sacola</h2>{cart.length === 0 ? <div className="py-10 text-center text-stone-500"><span className="text-4xl" aria-hidden>🛒</span><p className="mt-3 font-bold">Sua sacola está vazia</p><p className="mt-1 text-sm">Adicione produtos para começar.</p></div> : <>{cart.map((item) => <CartRow key={`${item._id}-${item.addonNames.join()}`} item={item} addonPrice={addonPrice} setQuantity={setQuantity} />)}<div className="mt-5 flex justify-between border-t pt-4 text-lg font-black"><span>Subtotal</span><span className="whitespace-nowrap">{money(subtotal)}</span></div><button type="button" onClick={checkout} className="mt-4 min-h-12 w-full rounded-xl bg-lime px-4 font-black text-ink">FINALIZAR PEDIDO</button></>}</section>;
}
...
function MobileCartBar({ count, subtotal, open }: { count: number; subtotal: number; open: () => void }) {
  return <aside className="fixed inset-x-0 bottom-0 z-40 border-t bg-white/95 px-4 pt-3 shadow-2xl backdrop-blur lg:hidden" style={{ paddingBottom: 'max(.75rem, env(safe-area-inset-bottom))' }}><button type="button" onClick={open} className="mx-auto flex min-h-14 w-full max-w-2xl items-center justify-between gap-3 rounded-xl bg-lime px-5 font-black text-ink"><span>🛒 VER SACOLA · {count} {count === 1 ? 'item' : 'itens'}</span><span className="whitespace-nowrap">{money(subtotal)}</span></button></aside>;
}

function ModalFrame({ label, close, children, sheet = false }: { label: string; close: () => void; children: React.ReactNode; sheet?: boolean }) {
  return <div className={`fixed inset-0 z-[70] overflow-y-auto bg-ink/60 p-0 ${sheet ? 'flex items-end sm:block sm:p-4' : 'p-3 sm:p-6'}`} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section role="dialog" aria-modal="true" aria-label={label} className={`mx-auto max-h-[92dvh] w-full overflow-y-auto bg-white shadow-2xl ${sheet ? 'max-w-2xl rounded-t-3xl p-5 sm:my-6 sm:rounded-3xl sm:p-7' : 'my-3 max-w-2xl rounded-3xl p-5 sm:my-6 sm:p-7'}`}>{children}</section></div>;
}
