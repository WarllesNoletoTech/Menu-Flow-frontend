...
export function TablesWorkspace({ ownerMode = false }: { ownerMode?: boolean }) {
  ...
  async function toggleServiceFee(enabled: boolean) {
    if (!detail) return;
    const label = enabled ? 'Taxa de serviço reativada.' : 'Taxa de serviço removida da conta.';
    await run(async () => {
      const updated = await authenticatedRequest<Session>(`/table-service/sessions/${detail._id}/service-fee`, {
        method: 'PATCH',
    ...
    {detail&&<Modal wide title={addingProduct?addingProduct.name:tableTitle(detail)} close={closeDetailModal}>
      ...
      {addingProduct ? <div className="mx-auto max-w-2xl space-y-4">
        ...
        {detailTab==='ACCOUNT'&&<div className="grid gap-3 lg:grid-cols-[.85fr_1.15fr_.9fr]">
          <section className="rounded-2xl bg-background p-4"><h3 className="font-black">🧾 Resumo da conta</h3><dl className="mt-3 space-y-2 text-sm"><Row label="Pedidos" value={money(detail.subtotalCents)}/><Row label={detail.serviceFeePercent > 0 ? `Serviço (${detail.serviceFeePercent}%)` : 'Serviço'} value={detail.serviceFeePercent > 0 ? money(detail.serviceFeeCents) : 'Removido'}/><Row label="Desconto" value={`- ${money(detail.discountCents)}`}/><Row label="Pago" value={money(detail.paidCents)}/><div className="border-t pt-2"><Row strong label="Total" value={money(detail.totalCents)}/><Row strong label="Saldo pendente" value={money(detail.balanceCents)}/></div></dl>{detail.status==='AWAITING_PAYMENT'&&can('TABLES_PAYMENT')&&<div className={`mt-4 rounded-2xl border p-3 ${detail.serviceFeePercent>0?'border-amber-200 bg-amber-50':'border-emerald-200 bg-emerald-50'}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><strong className={detail.serviceFeePercent>0?'text-amber-900':'text-emerald-900'}>{detail.serviceFeePercent>0?`Taxa de serviço ${detail.serviceFeePercent}% ativa`:'Taxa de serviço removida'}</strong><p className="mt-0.5 text-xs text-stone-600">O valor é recalculado imediatamente e fica registrado no histórico da mesa.</p></div><button type="button" disabled={busy} onClick={()=>void toggleServiceFee(detail.serviceFeePercent<=0)} className={`rounded-xl px-3 py-2 text-xs font-black ${detail.serviceFeePercent>0?'bg-white text-amber-900 ring-1 ring-amber-300':'bg-emerald-700 text-white'}`}>{detail.serviceFeePercent>0?'Remover taxa de serviço':'Reativar taxa de serviço'}</button></div></div>}<div className="mt-4 grid gap-2"><button type="button" disabled={busy} onClick={()=>void printBill(detail)} className="rounded-xl border bg-white px-4 py-3 font-black">Imprimir pré-conta</button>{detail.status==='OPEN'&&hasBillableOrders&&can('TABLES_REQUEST_BILL')&&<button type="button" disabled={busy} onClick={()=>void run(async()=>{const updated=await authenticatedRequest<Session>(`/table-service/sessions/${detail._id}/request-bill`,{method:'PATCH'});setDetail(updated)},'Conta enviada ao caixa.')} className="rounded-xl bg-gold px-4 py-3 font-black text-ink">🧾 Solicitar conta ao caixa</button>}{detail.balanceCents===0&&detail.status==='AWAITING_PAYMENT'&&can('TABLES_CLOSE')&&<button disabled={busy} onClick={()=>void run(()=>authenticatedRequest(`/table-service/sessions/${detail._id}/close`,{method:'PATCH'}),'Mesa fechada.').then(()=>{closeDetailModal()})} className="rounded-xl bg-success px-4 py-3 font-black text-white">Fechar mesa</button>}</div></section>
...
function browserPrint(title:string, lines:string[], paper:58|80=80) { const popup=window.open('','_blank','width=440,height=720'); if(!popup) return; const escaped=lines.map((line)=>escapeHtml(line)).join('\n'); const body=paper===58?52:74; const margin=paper===58?3:3; const font=paper===58?9.5:10.5; popup.document.write(`<html><head><title>${escapeHtml(title)}</title><style>@page{size:${paper}mm auto;margin:0}html,body{margin:0;padding:0;background:white}body{width:${paper}mm;box-sizing:border-box;padding:${margin}mm;font-family:Consolas,"Courier New",monospace;font-size:${font}px;line-height:1.25}pre{width:${body}mm;max-width:100%;margin:0;white-space:pre-wrap;overflow-wrap:break-word;word-break:normal}</style></head><body><pre>${escaped}</pre><script>window.onload=()=>{window.print();setTimeout(()=>window.close(),500)}<\/script></body></html>`); popup.document.close(); }
