...
export default function Page() {
  ...
  const [form, setForm] = useState({ minimumOrder: '', preparationMinutes: '', pickupEnabled: true, deliveryEnabled: false, serviceFeePercent: '10', qrOrderingEnabled: false, qrRequireWaiterApproval: true, open: true });
  ...
  useEffect(() => {
    if (settings && establishment) setForm({ minimumOrder: String(settings.minimumOrder ?? 0), preparationMinutes: String(settings.preparationMinutes ?? ''), pickupEnabled: settings.pickupEnabled ?? true, deliveryEnabled: settings.deliveryEnabled ?? false, serviceFeePercent: String(settings.serviceFeePercent ?? 10), qrOrderingEnabled: settings.qrOrderingEnabled ?? false, qrRequireWaiterApproval: settings.qrRequireWaiterApproval ?? true, open: establishment.open });
  ...
  async function save(event: FormEvent) {
    ...
    try {
      await Promise.all([
        request('/restaurants/me/settings', { method: 'PATCH', body: JSON.stringify({ minimumOrder: Number(form.minimumOrder), preparationMinutes: form.preparationMinutes ? Number(form.preparationMinutes) : undefined, pickupEnabled: form.pickupEnabled, deliveryEnabled: form.deliveryEnabled, serviceFeePercent: Number(form.serviceFeePercent), qrOrderingEnabled: form.qrOrderingEnabled, qrRequireWaiterApproval: form.qrRequireWaiterApproval }) }),
  ...
  return <section className="mx-auto max-w-3xl">
    ...
    <form onSubmit={save} className="mt-6 space-y-5 rounded-2xl bg-surface p-5 shadow-sm">
      ...
      <div><h3 className="font-black">Formas de atendimento</h3><div className="mt-2 grid gap-3 sm:grid-cols-2"><label className="flex items-center gap-3 rounded-xl border p-4 font-bold"><input type="checkbox" checked={form.pickupEnabled} onChange={e=>setForm({...form,pickupEnabled:e.target.checked})}/> Retirada no local</label><label className="flex items-center gap-3 rounded-xl border p-4 font-bold"><input type="checkbox" checked={form.deliveryEnabled} onChange={e=>setForm({...form,deliveryEnabled:e.target.checked})}/> Entrega</label></div>{!form.pickupEnabled&&!form.deliveryEnabled&&!settings?.tableServiceEnabled&&<p className="mt-2 text-sm font-bold text-danger">Ative pelo menos uma forma para receber pedidos.</p>}</div><div className="grid gap-4 sm:grid-cols-2"><label className="font-bold">Pedido mínimo (R$)<input min="0" step="0.01" type="number" className="field" value={form.minimumOrder} onChange={(event) => setForm({ ...form, minimumOrder: event.target.value })}/></label><label className="font-bold">Tempo de preparo (minutos)<input min="0" type="number" className="field" value={form.preparationMinutes} onChange={(event) => setForm({ ...form, preparationMinutes: event.target.value })}/></label></div>
      <div className="rounded-xl border p-4"><div className="flex items-start justify-between gap-4"><div><h3 className="font-black">Salão e garçom</h3><p className="mt-1 text-sm text-stone-500">{settings?.tableServiceEnabled ? 'Controle de mesas liberado pelo administrador.' : 'O administrador ainda não liberou o controle de mesas para esta loja.'}</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${settings?.tableServiceEnabled?'bg-success/15 text-success':'bg-stone-200 text-stone-600'}`}>{settings?.tableServiceEnabled?'Ativo':'Desativado'}</span></div>{settings?.tableServiceEnabled&&<div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="font-bold">Taxa de serviço (%)<input min="0" max="100" step="0.01" type="number" className="field" value={form.serviceFeePercent} onChange={(event)=>setForm({...form,serviceFeePercent:event.target.value})}/></label><label className="flex items-center justify-between rounded-xl border p-4 font-bold"><span>Preparar QR por mesa</span><input type="checkbox" checked={form.qrOrderingEnabled} onChange={(event)=>setForm({...form,qrOrderingEnabled:event.target.checked})}/></label>{form.qrOrderingEnabled&&<label className="flex items-center justify-between rounded-xl border p-4 font-bold sm:col-span-2"><span>Pedido do QR exige aprovação do garçom</span><input type="checkbox" checked={form.qrRequireWaiterApproval} onChange={(event)=>setForm({...form,qrRequireWaiterApproval:event.target.checked})}/></label>}</div>}</div>
      <button disabled={busy || (!form.pickupEnabled && !form.deliveryEnabled && !settings?.tableServiceEnabled)} className="rounded-xl bg-ink px-6 py-3 font-bold text-white disabled:opacity-50">{busy ? 'Salvando…' : 'Salvar configurações'}</button>
    </form>
