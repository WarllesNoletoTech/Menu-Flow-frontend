...
export default function Page() {
  ...
  const [form, setForm] = useState({ minimumOrder: '', preparationMinutes: '', rappidexEnabled: false, open: true });
  ...
  useEffect(() => {
    if (settings && establishment) setForm({ minimumOrder: String(settings.minimumOrder ?? 0), preparationMinutes: String(settings.preparationMinutes ?? ''), rappidexEnabled: settings.rappidexEnabled, open: establishment.open });
  ...
  async function save(event: FormEvent) {
    ...
    try {
      await Promise.all([
        request('/restaurants/me/settings', { method: 'PATCH', body: JSON.stringify({ minimumOrder: Number(form.minimumOrder), preparationMinutes: form.preparationMinutes ? Number(form.preparationMinutes) : undefined, rappidexEnabled: form.rappidexEnabled }) }),
  ...
  return <section className="mx-auto max-w-3xl">
    ...
    <form onSubmit={save} className="mt-6 space-y-5 rounded-2xl bg-surface p-5 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="font-bold">Pedido mínimo (R$)<input min="0" step="0.01" type="number" className="field" value={form.minimumOrder} onChange={(event) => setForm({ ...form, minimumOrder: event.target.value })}/></label><label className="font-bold">Tempo de preparo (minutos)<input min="0" type="number" className="field" value={form.preparationMinutes} onChange={(event) => setForm({ ...form, preparationMinutes: event.target.value })}/></label></div>
      ...
      <button disabled={busy} className="rounded-xl bg-ink px-6 py-3 font-bold text-white disabled:opacity-50">{busy ? 'Salvando…' : 'Salvar configurações'}</button>
    </form>
    <section className="mt-6 rounded-2xl bg-surface p-5 shadow-sm"><h3 className="text-lg font-black">Entrega</h3><p className="text-sm text-stone-500">A entrega só será oferecida onde houver uma região ativa.</p><form onSubmit={addZone} className="mt-4 grid gap-3 sm:grid-cols-[1fr_160px_auto]"><input required className="field !mt-0" placeholder="Bairro ou região" value={zone.name} onChange={(event) => setZone({ ...zone, name: event.target.value })}/><input required min="0" step="0.01" type="number" className="field !mt-0" placeholder="Taxa (R$)" value={zone.fee} onChange={(event) => setZone({ ...zone, fee: event.target.value })}/><button disabled={busy} className="rounded-xl bg-ink px-4 font-bold text-white disabled:opacity-50">Adicionar</button></form><div className="mt-4 space-y-2">{operations.deliveryZones.map((item) => <div key={item._id} className="flex items-center justify-between rounded-xl border p-3"><span><b>{item.name}</b> · {item.fee.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span><button disabled={busy} onClick={() => void toggleZone(item)} className="font-bold underline">{item.active ? 'Desativar' : 'Ativar'}</button></div>)}</div></section>
