...
export default function Page() {
  ...
  const [form, setForm] = useState({ minimumOrder: '', preparationMinutes: '', pickupEnabled: true, deliveryEnabled: false, open: true });
  ...
  useEffect(() => {
    if (settings && establishment) setForm({ minimumOrder: String(settings.minimumOrder ?? 0), preparationMinutes: String(settings.preparationMinutes ?? ''), pickupEnabled: settings.pickupEnabled ?? true, deliveryEnabled: settings.deliveryEnabled ?? false, open: establishment.open });
  ...
  async function save(event: FormEvent) {
    ...
    try {
      await Promise.all([
        request('/restaurants/me/settings', { method: 'PATCH', body: JSON.stringify({ minimumOrder: Number(form.minimumOrder), preparationMinutes: form.preparationMinutes ? Number(form.preparationMinutes) : undefined, pickupEnabled: form.pickupEnabled, deliveryEnabled: form.deliveryEnabled }) }),
