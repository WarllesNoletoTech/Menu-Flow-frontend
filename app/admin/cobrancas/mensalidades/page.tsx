...
const previousPeriod = () => { const now = new Date(); const d = new Date(now.getFullYear(), now.getMonth() - 1, 1); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`; };
...
export default function Page() {
  ...
  async function generate() {
    ...
    try {
      const result = await request<{ results: Array<{ created?: boolean; alreadyExists?: boolean }> }>('/billing/invoices/generate', { method: 'POST', body: JSON.stringify({ period }) });
  ...
  const filtered = useMemo(() => list.items.filter((item) => !period || item.period === period), [list.items, period]);
  ...
  return <section className="mx-auto max-w-7xl space-y-6">
    <header className="overflow-hidden rounded-[32px] bg-gradient-to-r from-ink via-[#6f1c21] to-[#8f242a] p-6 text-white shadow-xl sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-lime">Financeiro da plataforma</p><h1 className="mt-2 text-3xl font-black">Cobranças por faturamento</h1><p className="mt-2 max-w-2xl text-sm text-white/75">Mensalidade automática conforme o faturamento do mês anterior, sem comissão por pedido.</p></div><Link href="/admin/cobrancas/planos" className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-sm font-black">⚙️ Configurar faixas</Link></div>
    ...
    <section className="rounded-[28px] border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-primary">Gerar mensalidades</p><h2 className="mt-1 text-xl font-black">Fechamento mensal</h2><p className="mt-1 text-sm text-stone-500">O sistema usa produtos vendidos menos descontos. Entrega e taxa do garçom ficam fora.</p></div><div className="flex flex-wrap items-end gap-2"><label className="text-sm font-bold">Mês de referência<input type="month" className="field min-w-44" value={period} onChange={(e) => setPeriod(e.target.value)} /></label><button disabled={busy || !period} onClick={() => void generate()} className="min-h-12 rounded-2xl bg-ink px-5 py-3 text-sm font-black text-white disabled:opacity-50">{busy ? 'Processando…' : 'Gerar cobranças'}</button></div></div>
      ...
      <p className="mt-3 text-xs text-stone-500">Vencimento padrão: primeira terça-feira do mês seguinte à referência.</p>
